...
def main():
    ...
    print("Connected to device 0x%04X/0x%04X" %
          (hid_device.idVendor, hid_device.idProduct))

    p = 0
    while (True):
        send_led_pattern(hid_device, 1, 2, 3, 4)
        # Receive and print the current LED pattern
        received = receive_led_pattern(hid_device)[0:20]
        print("Received LED Pattern: {0}".format(received))
